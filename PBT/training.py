...
def generate_params():
    ...
    return {
        ...
        'load_checkpoint_dir': "model_checkpoint",
        "save_checkpoint_dir": "model_checkpoint"
    }


def training(params):

    # get path for model to load, parameter "load_checkpoint_dir" is delivered by nni
    load_path = os.path.join(params['load_checkpoint_dir'], 'model.p')

    # check if path already exists and load model
    if os.path.isfile(load_path):
        booster = pickle.load(open(load_path, 'rb'))
    ...
    # is path does not exist, train new model
    else:
        booster = lgb.train(params, feval=my_accuracy, train_set=train, valid_sets=val_1,
                            early_stopping_rounds=params["early_stopping_rounds"],
                            num_boost_round=params["num_boost_round"])

    # get accuracy form val_2
    preds = booster.predict(val_2.data)
    score = accuracy_score(val_2.label.astype("int"), np.argmax(preds, axis=1))
    nni.report_final_result(score)

    # save model under "save_checkpoint_dir"
    save_path = os.path.join(params['save_checkpoint_dir'], 'model.p')

    # check if folder already exists
    if os.path.isdir(params['save_checkpoint_dir']):
        pickle.dump(booster, open(save_path, 'wb'))
    else:
